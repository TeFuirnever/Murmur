...
| **3.1** Migrate 75 `.js` test files              | 1-2 weeks   | **53/54 migrated** (only `regression-session-fixes.test.js` remains — 666 lines, 30 require sites, "project's most fragile infra"). Pattern proven across 5 batches: per-file strict-typing work via `let x: typeof import("...").<export>` + structural `*Surface` interfaces for private access + `MockIpcMain`/`ReturnType<typeof vi.fn>` for vitest 4. See §5.0.1 for per-file rationale. | ✅ Near-done |
