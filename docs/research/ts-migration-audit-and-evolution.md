...
| **4.1** Migrate root configs `.js`→`.ts`         | 0.5 day     | **Partial.** `playwright.config.js→.ts` done (PR #108). Remaining: `vitest.config.js`, `postcss.config.js`, `tailwind.config.js`, `cleanup.js`, `scripts/ci-check.js`.                                                                                                                                                                                                                                      | 🟡 Partial  |
| **4.2** Adopt `@electron-toolkit/typed-ipc`      | 1 week      | Not started. Current `.d.ts` quality is high; ROI low.                                                                                                                                                                                                                                                                                                                                                      | ⏸ Deferred  |
| **4.3** Extract `tests/e2e/` to TS               | 2-3 days    | **DONE.** PRs #108 + #109 migrated 4 helpers + 13 suites + `playwright.config.ts`. 4 `.js` remain (`ci-probe.js` — Electron `--require` constraint; 3 `tests/e2e/legacy/*.test.js` — not in active suite).                                                                                                                                                                                                  | ✅ Done     |
