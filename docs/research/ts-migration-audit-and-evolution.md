...
| **3.1** Migrate 75 `.js` test files              | 1-2 weeks   | **17/54 migrated** (smoke, determineProcessingMode, phase0-7, edge-cases, ci-config, windows-compat, main-process-module-resolution, updateManager-require-resolution, static-import-audit, jsx-extension-guard, usePermissions). Pattern proven: per-file strict-typing work via `let x: typeof import("...").<export>` + non-null assertions after length checks. See §5.0.1 for skip-list rationale. | ⚠️ Partial  |
...
Updated 2026-07-26 after batches 3+4 (PRs #95, #96) brought the count from 54 → 6. The 3 original entries are marked with their resolution status; 5 new entries added for batch-5 holdouts.

| File                                                                | Reason for deferral / resolution                                                                                                                                                                                                                                                     | Status      |
| ------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------- |
| `regression-session-fixes.test.js` (666 lines)                      | **Effort-reward.** Largest test file in the suite; uses `vi.mock("electron")` + 30 `require()` sites — the audit doc (line 213) calls this "the project's most fragile infra." Migration requires replacing the mock-electron strategy with a typed mock first. Not a 1-day task.    | ⛔ Deferred |
| `dynamicTranscriptionTimeout.test.js`                               | **RESOLVED in batch 3.** Originally thought module-private; investigation showed `calculateTranscriptionTimeout` is mounted as a public static on the default-exported `FunASRServer` class. Typed via `typeof import("...").default.calculateTranscriptionTimeout`.                 | ✅ Migrated |
| `assert-electron-api.test.js`                                       | **RESOLVED in batch 3.** Used `Omit<Window, "electronAPI"> & { electronAPI?: ElectronAPI }` pattern (per batch-2's `usePermissions.test.ts`).                                                                                                                                        | ✅ Migrated |
| `aiHandlers.test.js` (328 lines, 25 vi, 1 electron mock)            | **Heavy vi.mock + electron dependency.** 25 `vi.*` calls + `vi.mock("electron")`. Mock-electron strategy needs typed surface first. Same root blocker as `regression-session-fixes`.                                                                                                 | ⛔ Deferred |
| `funasrServer-crash-restart.test.js` (108 lines, 10 vi, 1 electron) | **ChildProcess mock + electron.** Spawns MockProcess extending EventEmitter; needs typed electron mock for graceful shutdown paths.                                                                                                                                                  | ⛔ Deferred |
| `ipcRateLimitIntegration.test.js` (114 lines, 28 vi, 1 electron)    | **Heaviest vi.mock density.** 28 `vi.*` calls in 114 lines + electron mock. Migration would require an electron-mock pattern that doesn't exist yet.                                                                                                                                 | ⛔ Deferred |
| `logManager.test.js` (125 lines, 4 vi, 1 electron)                  | **Light electron dependency.** Uses `require("electron")` lazily inside source for `app.getPath("userData")`. Migration is feasible — the electron mock can be typed structurally — but the file touches the `electron.app` API which the project hasn't built a typed mock for yet. | ⛔ Deferred |
| `windowManager-events.test.js` (175 lines, 17 vi, 1 electron)       | **BrowserWindow mock + electron.** Mocks `electron.BrowserWindow` events (`maximize`, `unmaximize`, etc.). Same electron-mock blocker as `aiHandlers`.                                                                                                                               | ⛔ Deferred |

**Common thread for the 6 deferred files:** 5 of 6 need a typed electron-mock pattern that doesn't exist in the codebase yet. The right next step is to design ONE reusable electron mock (covering `app`, `BrowserWindow`, `ipcMain`, `Tray`, `globalShortcut`) and migrate all 5 in a single batch. `regression-session-fixes` would benefit from the same mock but is additionally the largest file (666 lines, 30 require sites).

These 6 files are tracked; they are not oversights.
