...
| File                                           | Reason for deferral                                                                                                                                                                                                                                                                                         | Batch target |
| ---------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------ |
| `regression-session-fixes.test.js` (666 lines) | **Effort-reward.** Largest test file in the suite; uses `vi.mock("electron")` + `require()` chains the audit doc (line 213) calls "the project's most fragile infra." Migration requires replacing the mock-electron strategy with a typed mock first. Not a 1-day task.                                    | Batch 4+     |
| `dynamicTranscriptionTimeout.test.js`          | **Module-private blocker.** Test imports `calculateTranscriptionTimeout` which is not exported from `funasrServer.ts`. ESM `import` cannot reach it without changing the source export shape. Deferring until either the source exports the helper OR the test is rewritten to exercise the public surface. | Batch 3      |
| `assert-electron-api.test.js`                  | **Window-type conflict.** Uses `globalThis.window = {}` to test the assertion; strict tsc's `Window` type makes `{}` incompatible. Requires either `as unknown as Window` cast or a different stub strategy. Pattern documented in batch-2's `usePermissions.test.ts` migration (uses `Omit<Window, ...>`). | Batch 3      |

These three files are tracked; they are not oversights. The remaining 34 unmigrated `.js` tests are unstarted, not blocked, and follow the batch-1 pattern with expected effort of 5-30 minutes each.
