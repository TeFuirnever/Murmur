...
| **3.2** Delete `_tsresolve.setup.js`             | 0.5 day     | **Still blocked.** Although 3.1 is complete, 96 `require()` sites remain across the .ts tests (23 files use `vi.resetModules()` for per-test module isolation, which depends on the shim's CJS-require unwrapping). Conversion requires rethinking those 23 files to use `vi.doMock()` + dynamic `await import()` instead of synchronous require. Realistic effort: 3-5 days (NOT 0.5 day as originally estimated). | ⛔ Blocked  |
| **3.3** Re-enable `no-require-imports: error`    | 1 week      | Not started. Depends on 3.2.                                                                                                                                                                                                                                                                                                                                                                                        | ⛔ Blocked  |
