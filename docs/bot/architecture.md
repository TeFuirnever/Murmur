# Architecture (bloub bot port)

Ported note, adapted for Murmur. Upstream original:
[jeremy-prt/bloub](https://github.com/jeremy-prt/bloub), `docs/architecture.md`
(MIT). Section names and all measured reasoning preserved; "the component"
refers to the host shell (upstream: BloubBot.vue; Murmur: BloubBot.tsx), and
identifier spellings reflect the Englishified port (RADIUS, HALF_VIEWBOX,
eyeShift, ...). See also [measurements.md](./measurements.md) and the
absorption audit in `docs/research/2026-09-05-bloub-absorption-audit.md`.
...
only one slot of history, so the origin of the new blend used to become the _full_ pose of
...
mid-course — and there is nothing to fix outside a fade, where the state being left _is_
...
- `MIN_BLOCK`, **derived** from the longest `morph` in the catalogue (`orbit`, 0.6 s),
  ...
  that maximum.
  It no longer exists to prevent a jump: a state change landing _inside_ a fade now
...
`wide`, `notify` and `swirl`. Everywhere else the silhouette _is_ the animation
...
and the burst particles _are_ drawn behind the body to be occluded by it, the body
...
- **Scaling the face.** Scaling position _and_ eye size keeps every ratio and is stable,
...
than speed — a morph moves the eyes fast anyway, trembling is going _and coming back_ — it
...
arc in two, and the back half is drawn _before_ the body so the body occludes it.
...
done by the engine because only it knows the pose _at instant t_. A caller
...
`spin` is a turn taken _on the way_: free on a sphere, and with no effect on the
...
stay turned _and_ keep living. Conflating them froze the gaze the moment the view
