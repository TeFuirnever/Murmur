// [20260724_TS_BigBang_PythonInstaller] Migrated from .js to .ts (ADR-010).
// `module.exports = PythonInstaller` became `export default PythonInstaller`.
// No electron dependency — only fs/path/https/os and runCommand from utils.
import fs from "fs";
...
interface PythonInstalledResult {
  installed: boolean;
  command?: string;
  version?: number;
}
...
class PythonInstaller {
  ...
  async downloadFile(
    url: string,
    outputPath: string,
    progressCallback: ((progress: DownloadProgress) => void) | null = null,
  ): Promise<void> {
  ...
  async checkWindowsAdmin(): Promise<boolean> {
    try {
      // 尝试读取受保护的注册表项来检查管理员权限
      await runCommand("reg", ["query", "HKU\\S-1-5-19"], {
        timeout: TIMEOUTS.QUICK_CHECK,
      });
      return true;
    } catch {
  ...
  async installPythonWindows(
    ...
    try {
      ...
      await runCommand(installerPath, installArgs, {
        timeout: TIMEOUTS.INSTALL,
      });
  ...
  async installPythonLinux(
    ...
    try {
      ...
      try {
        ...
        await runCommand("apt", ["--version"], {
          timeout: TIMEOUTS.QUICK_CHECK,
        });

        if (progressCallback) {
          progressCallback({
            stage: "通过 apt 安装 Python...",
            percentage: 30,
          });
        }
  ...
  async isPythonInstalled(): Promise<PythonInstalledResult> {
    ...
    for (const fullPath of additionalPaths) {
      try {
        if (fs.existsSync(fullPath)) {
          const result = await runCommand(fullPath, ["--version"], {
            timeout: TIMEOUTS.QUICK_CHECK,
          });
          const versionMatch = result.output.match(/Python (\d+\.\d+)/);
          if (versionMatch) {
