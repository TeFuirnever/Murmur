// [20260724_TS_BigBang_Clipboard] Migrated from .js to .ts (ADR-010).
// `module.exports = ClipboardManager` (class) became `export default
// ClipboardManager`. The conditional `require("osascript")` inside a
// try/catch is INTENTIONALLY kept as require() — `import` is hoisted and
// would lose the error handling (special case 1).
...
class ClipboardManager {
  ...
  constructor(logger: Logger | null) {
    ...
    if (process.platform === "darwin") {
      try {
        // KEEP as require() — import is hoisted and would lose the
        // try/catch error handling for the optional osascript dependency.
  ...
  safeLog(message: string, data: unknown = null): void {
    if (this.logger) {
      try {
        ...
        if ((error as NodeJS.ErrnoException).code !== "EPIPE") {
          process.stderr.write(`日志错误: ${(error as Error).message}\n`);
        }
      }
    }
  }
  ...
  async enableMacOSAccessibility(): Promise<boolean> {
  ...
  async insertTextDirectly(text: string): Promise<unknown> {
    // 简化实现，直接使用标准的粘贴方法
  ...
  async pasteMacOS(originalClipboard: string): Promise<unknown> {
  ...
  async pasteWindows(originalClipboard: string): Promise<unknown> {
    return new Promise((resolve, reject) => {
  ...
  async pasteLinux(originalClipboard: string): Promise<unknown> {
    return new Promise((resolve, reject) => {
  ...
  showAccessibilityDialog(testError: string): void {
...
您想现在打开系统设置吗？`;
  ...
  openSystemSettings(): void {
    const settingsCommands: Array<[string, string[]]> = [
      [
        "open",
        [
  ...
  /** 复制文本到剪贴板 */
  async copyText(text: string): Promise<{ success: boolean }> {
    try {
      clipboard.writeText(text);
      return { success: true };
    } catch (error) {
