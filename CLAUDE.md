...
When adding platform-specific code, use `process.platform === "win32"` checks — in the **main process and plain-Node code only**. Sandboxed renderer pages (contextIsolation, no nodeIntegration) have no `process` global; a bare `process.platform` in renderer code survives the bundle and throws at render time (jsdom tests cannot catch it — Node provides `process`). Gate renderer UI on the preload bridge's `window.electronAPI.getPlatform()` instead. Add tests with `it.skipIf(process.platform === "win32")` for Unix-only behavior.
...
- **All commits MUST pass `pnpm ci:check` before push.** This mirrors CI and runs: format check, lint, debt-marker check, license check, typecheck, typecheck:tests, test with coverage, build:main, build:preload, build:renderer, dev smoke (pnpm run dev boots and the vite dev server answers).
